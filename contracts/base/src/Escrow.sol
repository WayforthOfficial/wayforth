...
/// - No admin pause: routePayment() cannot be stopped by any admin action.
/// - No owner withdrawal: admin cannot extract user funds or service owner funds.
/// - No fund redirection: updateFeeRecipient() only affects Wayforth's fee share,
///   never the serviceOwner's payment or the user's refund path.
/// - rescueToken() has been removed. Tokens sent directly to this contract
///   (not via routePayment) are unrecoverable by design.
contract WayforthEscrow {
    ...
    /// @dev    Only affects Wayforth's fee destination. Cannot redirect user funds —
    ///         those go directly to serviceOwner in routePayment.
